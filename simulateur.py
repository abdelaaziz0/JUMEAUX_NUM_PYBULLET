...
"""
simulateur_complex.py (Option B - sans gripper)
----------------------------------------------
...
# Paramètres
NUM_CUBES = 3           # Nombre d'objets à ramasser
TABLE_POS = [0.7, 0.0, -0.2]  # Position de la table
PICK_HEIGHT = 0.025     # Hauteur à laquelle on considère la "prise" du cube
ROBOT_URDF = "kuka_iiwa/model.urdf"  # Robot Kuka IIWA (7 articulations, pas de pince)
...
def set_arm_joints(robot_id, joint_poses, max_force=300):
    ...
    for i in range(7):  # 7 articulations
...
def run_advanced_sim(
    ...
    # 1. Connexion PyBullet (GUI pour voir localement)
    ...
    # Charger le robot Kuka IIWA (7 DOF, pas de pince)
    robot_id = p.loadURDF(
        ROBOT_URDF, 
        [0.0, 0.0, 0.0], 
    ...
    end_effector_index = 6  # Sur Kuka IIWA, le 7e joint (index=6) est l'effecteur

    # 3. Générer plusieurs cubes
    cubes_data = []
    ...
    # Destination (zone de dépose)
    place_pos = [TABLE_POS[0], TABLE_POS[1] + 0.3, TABLE_POS[2] + 0.02]

    # Socket UDP
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

    print(f"[Simulateur Avancé - No Gripper] Lancement, envoi vers {udp_ip}:{udp_port}")

    step_count = 0
    current_cube_index = 0
    phase = "go_pick"
    teleporting_object = False  # Indique si on "tient" virtuellement l'objet
    ...
    while step_count < total_steps:
        ...
        # -- Position d'effecteur
        eff_state = p.getLinkState(robot_id, end_effector_index)
        eff_pos = eff_state[0]
        eff_orn = eff_state[1]

        # -- Vérifier s'il y a encore des cubes à traiter
        if current_cube_index >= NUM_CUBES:
            phase = "done"

        if phase != "done":
            # On traite un cube à la fois
            ...
            if phase == "go_pick":
                # Approche du cube (depuis une position de repos, par ex. [0.5,0,0.3])
                ...
                for pt in path:
                    ...
                    p.stepSimulation()
                    time.sleep(step_time)
                # Une fois descendu, on "tient" le cube (téléport)
                ...
                # Aller à la zone de dépose (en horizontal, puis descendre)
                current_eff = eff_pos
    ...
    p.disconnect()
    print("[Simulateur Avancé - No Gripper] Simulation terminée")
